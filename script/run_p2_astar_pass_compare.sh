...
OUT_ROOT="${OUT_ROOT:-data/BenchmarK/p2_astar/pass/pass_B${B_PASS}_${TAG}}"
...
BASE_PREFIX="${OUT_ROOT}/astar_base_s${SEEDS}_B${B_PASS}_${TAG}"
V2_PREFIX="${OUT_ROOT}/astar_hint_v2_s${SEEDS}_B${B_PASS}_${TAG}"
V3_PREFIX="${OUT_ROOT}/astar_hint_v3_s${SEEDS}_B${B_PASS}_${TAG}"

echo "== A* 通过预算正式对比开始 =="
echo "ROOT_DIR=$ROOT_DIR"
echo "B_PASS=$B_PASS SEEDS=$SEEDS SEED_START=$SEED_START TAG=$TAG"
...
SUCCESS_ONLY_CSV="${OUT_ROOT}/pass_compare_success_only.csv"
python3 "$ANALYZER" \
  --runs \
...
SUMMARY_CSV="${OUT_ROOT}/pass_compare_summary.csv"
